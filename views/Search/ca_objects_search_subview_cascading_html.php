...
?>
				<span class='multisearchFullResults'><?php print caNavLink($this->request, '<i class="fas fa-list" aria-label="list"></i> '._t('Full results'), '', '', 'Search', '{{{block}}}', array('search' => str_replace("/", "", $vs_search))); ?></span> 
<?php
				}
?>
...
<?php
		...
		while($qr_results->nextHit()) {
				...
				$vs_extended_info = $qr_results->getWithTemplate('<small><ifdef code="ca_objects.unitdate.dacs_date_text"><ifdef code="ca_objects.unitdate.dacs_date_text"><unit relativeTo="ca_objects.unitdate" delimiter=", "><ifdef code="ca_objects.unitdate.dacs_dates_labels">^ca_objects.unitdate.dacs_dates_labels: </ifdef>^ca_objects.unitdate.dacs_date_text<ifdef code="ca_objects.unitdate.dacs_dates_types"> ^ca_objects.unitdate.dacs_dates_types</ifdef></unit><ifdef code="ca_objects.material_type|ca_objects.extentDACS.extent_number|ca_objects.extentDACS.extent_type">, </ifdef></ifdef>
				...
				$vs_label_detail_link 	= caDetailLink($this->request, $qr_results->get("ca_objects.preferred_labels.name")."<br/>".$vs_extended_info, '', 'ca_objects', $vn_id);
				...
				if(!$this->request->getParameter("openResultsInOverlay", pInteger)){
					...
					$vs_rep_detail_link = "<a href='#' onclick='caMediaPanel.showPanel(\"".caNavUrl($this->request, 'Detail', 'resources', $vn_id, array('overlay' => 1))."\"); return false;'>".$vs_thumbnail."</a>";
				}
				$vs_add_to_set_link = "<div class='addTo'><a href='#' onclick='caMediaPanel.showPanel(\"".caNavUrl($this->request, '', 'Lightbox', 'addItemForm', array('object_id' => $vn_id))."\"); return false;' title='Add to lightbox'><i class='fas fa-plus-circle'></i></a></div>";
