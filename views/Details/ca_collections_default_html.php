...
?>				
			...
			<label>People</label>
						...
						foreach($qr_entities as $qr_entity){
							if($v_ents == 0) {print "<div class='row'>";}
...
?>
			...
			// Define the mimetypes dictionary
			$mimetypes = [
				'image' => ['image/jpeg', 'image/tiff', 'image/png', 'image/x-dcraw', 'image/x-psd', 'image/x-dpx', 'image/jp2', 'image/x-adobe-dng', 'image/bmp', 'image/x-bmp'],
				'video' => ['video/x-flv', 'video/mpeg', 'audio/x-realaudio', 'video/quicktime', 'video/x-ms-asf', 'video/x-ms-wmv', 'application/x-shockwave-flash', 'video/x-matroska', 'video/mp4'],
				...
				'file-audio' => ['audio/mpeg', 'audio/x-aiff', 'audio/x-wav', 'audio/wav', 'audio/mp4']
			];
			...
			foreach ($mimetype_counts as $mimetype => $count) {
				foreach ($mimetypes as $category => $mimetype_list) {
					if (in_array($mimetype, $mimetype_list)) {
			...
			<div class="row">
				<div class='counter'>
					...
					foreach ($category_counts as $category => $count) {
						echo "<div class='mimetypeCat'><i class='fas fa-" . strtolower($category) . "'></i><div class='value' akhi='$count'>0</div></div>";
					}
					?>
				</div>
			</div>
